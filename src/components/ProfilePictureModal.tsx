...
interface ProfilePictureModalProps {
  ...
  imageUrl: string;
  userName: string;
  isViewerOnly?: boolean; // Nova prop
}

const ProfilePictureModal: React.FC<ProfilePictureModalProps> = ({ isOpen, onClose, onChange, imageUrl, userName, isViewerOnly = false }) => {
      ...
      <div className="relative max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
        ...
        {!isViewerOnly && (
         ...
         <button onClick={onClose} className="absolute top-4 left-4 bg-white/80 backdrop-blur-sm text-slate-800 rounded-full p-2 hover:bg-white transition-colors">
            <svg xmlns="http://www.w3.org/20ac0/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
        </button>
      </div>
    </div>,
    document.body
